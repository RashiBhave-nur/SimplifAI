...
class create_issue(abstract_subcommand):
    ...
    def run(self, conn, config):
        ...
        if 'due-date' in config and config['due-date'] is not None:
            if config['due-date'] == 'tomorrow':
                tomorrow = datetime.date.today() + datetime.timedelta(days=1)
                issue_dict['duedate'] = tomorrow.strftime('%Y-%m-%d')
            else:
        ...
        try:
            ...
            if issue is None:
                raise RuntimeError('Failed to add an issue')
            ...
            if config['verbose']:
                get_logger().debug('Error creating issue: {}'.format(str(e)))
            raise
